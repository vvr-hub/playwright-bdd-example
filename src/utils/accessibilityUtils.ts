...
export async function clearAccessibilityReportsDirectory(): Promise<void> {
  if (fs.existsSync(reportDirAbsolute)) {
    fs.rmSync(reportDirAbsolute, { recursive: true, force: true });
    console.log(`Cleared accessibility reports directory: ${reportDirAbsolute}`);
  }
  fs.mkdirSync(reportDirAbsolute, { recursive: true });
  console.log(`Created accessibility reports directory: ${reportDirAbsolute}`);
}
